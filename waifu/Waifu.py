...
class Waifu():
    ...
    def __init__(self,
        ...
        self.shuoshuo_prompt = SystemMessage(content=f'你现在是一个想要发QQ空间或朋友圈的用户，请你根据以下内容，含蓄深沉隐晦的表达你觉得害羞或者该隐藏的内容\n或者大方，热烈地直抒你的胸意\n（请注意，这部分不再是对话，而是希望你代入一名人的心理来抒发感情）')
    ...
    def ss(self,message:Message) :
        ...
        if '#发送说说' in message.message:
                ...
                ed=message.message.find('#原文')
                if ed!=-1:
                    logging.info(f'按原文发出')
                    reply= message.message[ed+3:len(message.message)]
                else :
                    reply= message.message[5:len(message.message)]
                    if self.use_search:
